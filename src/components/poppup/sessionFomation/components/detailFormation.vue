...
<div class="flex flex-col">
    ...
    <div v-else class="flex  py-3 justify-between mt-4 px-7 w-full flex-col   border-[1px] border-[#63B6B9] rounded-md">
        <div class="flex flex-col justify-between">
            ...
            <div class="flex mt-3">
                <btn_ class=" bg-stone-300 rounded-md" :options="{label:'Condition d’accès au cours.pdf',ico2Style:'color_base',style:' py-1 text-red-500 w-full',stylelabel:' text-black',ico:$store.state.icons.pdf_,ico2:$store.state.icons.cloudDown}"></btn_>
            </div>
        </div>
    </div>
    <div class="flex flex-col  mt-7 text_xs">
        <span class="text_xs font-semibold  color_base">Quelques modèles</span>
        <div class="flex flex-row mt-6">
            <div class="flex flex-col mr-2">
                <span>Modèle de convocation</span>
                <btn_ class="rounded-md" :options="{label:'Modèle de convocation.pdf',ico2Style:'color_base',style:' py-1 text-red-500 w-full',stylelabel:' text-black',ico:$store.state.icons.pdf_,ico2:$store.state.icons.cloudDown}"></btn_>
            </div>
            <div class="flex flex-col ml-2">
                <span>Modèle d’attestation de formation</span>
                <btn_ class="rounded-md" :options="{label:'Condition d’accès au cours.pdf',ico2Style:'color_base',style:' py-1 text-red-500 w-full',stylelabel:' text-black',ico:$store.state.icons.pdf_,ico2:$store.state.icons.cloudDown}"></btn_>
            </div>
        </div>
        <div class="flex flex-row mt-6">
            <div class="flex flex-col">
    ...
    <div class="flex w-full text_xs mt-4">
        <table class=" w-full items-start px-1">
            ...
            <tr v-for="i in 4" :key="i" class=" duration-200 hover:text-white  hover:bg-[#63B6B9]  ">
                ...
                <td class="pl-5" v-text="'Ligne'+i"></td>
                <td class="pl-5 ">
                    <div class=" flex my-1 color_base" v-text="'{public}'"></div>
                </td>
                ...
                <td class=" px-2 text-teal-500">
                    <div class=" flex flex-row items-center py-1">
                        <button class=" mr-2 bg-transparent flex flex-row items-center px-1 rounded-md py-1 ">
                            <svg class=" fill-current color_base rounded-md w-4" viewBox="0 0 24 24"><path d="M20.71 7.04c.39-.39.39-1.04 0-1.41l-2.34-2.34c-.37-.39-1.02-.39-1.41 0l-1.84 1.83 3.75 3.75M3 17.25V21h3.75L17.81 9.93l-3.75-3.75L3 17.25z" /></svg>
                            <u class=" ml-1 text_xs color_base">Modifier</u>
                        </button>
